    ...
    return (
        <main className='w-full h-screen flex justify-center items-center bg-[#f6f8fc] relative'>
            ...
            {isSidebarOpen || windowWidth > 1281 ? (
                <Sidebar activeSidebarItem={activeSidebarItem} handleSidebarItemClick={handleSidebarItemClick} setIsSidebarOpen={setIsSidebarOpen} isSidebarOpen={isSidebarOpen} />)
                :
                windowWidth > 1024 ?
...
type MyComponentProps = {
    ...
    handleSidebarItemClick: (itemName: string) => void;
    setIsSidebarOpen: React.Dispatch<React.SetStateAction<boolean>>;
};

const Sidebar: React.FC<MyComponentProps> = ({ activeSidebarItem, handleSidebarItemClick, isSidebarOpen, setIsSidebarOpen }) => {
            ...
            <div ref={sidebarRef} className='h-full w-72 lg:fixed xl:static min-w-72 xl:w-80 xl:min-w-80 top-0 left-0 z-50  bg-white border-r-2'>
                <div className='w-full h-full flex flex-col relative'>
                    ...
                    <ul className='w-full flex items-center justify-center flex-col mt-10'>
                        <li
                            className={`w-[90%] cursor-pointer text-xl font-semibold py-4 px-2 flex items-center mb-2 rounded-[10px] 
...
${activeSidebarItem === "Settings" ? "bg-[#7650e0] text-white" : "text-gray-600 hover:bg-[#7650e0] hover:text-white"}`}
                    ...
                    <div className='relative flex w-full justify-center items-center flex-col mt-6'>
                        ...
                        <div className='bg-[#f4f0fd] w-[85%] xl:w-[75%] h-48 mt-[-18px] z-10 rounded-[8px] flex flex-col items-center justify-center'>
                            ...
                            <p className=' w-[95%] flex flex-wrap items-center justify-center text-center text-gray-600  mt-2'>Start your skincare  with a quick appointment.</p>
                            <button className="bg-[#8f67e2] hover:bg-[#9c75e9] py-2 px-7 font-semibold  text-white rounded-[8px] mt-5 ">Book now</button>
                        </div>
                    </div>
                    <div className='bottom-0 absolute flex items-center justify-center w-full '>
                        <p
                            className={`w-[90%] cursor-pointer text-xl font-semibold py-4 px-2 flex items-center mb-2 rounded-[10px] text-gray-600 hover:bg-[#7650e0] hover:text-white`}
                        >
